...
public interface EquipoRepository extends JpaRepository<Equipo, Integer> {
    ...
    @Query("SELECT e FROM Equipo e LEFT JOIN FETCH e.simReferenciada LEFT JOIN FETCH e.plataforma WHERE e.tipo IN ('VENDIDO', 'DEMO') ORDER BY e.nombre")
        // Agregado FETCH e.plataforma
    ...
    @Query("""
                ...
                WHERE e.tipo IN ('VENDIDO', 'DEMO') 
                AND (
                    (e.estatus = 'ACTIVO' AND CAST(e.fechaExpiracion AS date) BETWEEN CURRENT_DATE AND CAST(CURRENT_DATE + 30 DAY AS date))
                    OR 
                    (e.estatus = 'EXPIRADO')
                )
                ORDER BY e.fechaExpiracion ASC
