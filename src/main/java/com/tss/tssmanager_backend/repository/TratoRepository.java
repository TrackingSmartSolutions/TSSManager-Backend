...
SELECT 
    ...
    t.fecha_ultima_actividad,
    u.nombre as propietario_nombre,
    e.nombre as empresa_nombre, e.domicilio_fisico, e.sitio_web, 
...
""", nativeQuery = true)
    ...
    @Query("SELECT t FROM Trato t WHERE t.empresaId = :empresaId AND t.fase IN ('COTIZACION_PROPUESTA_PRACTICA', 'NEGOCIACION_REVISION', 'CERRADO_GANADO', 'INTERES_FUTURO', 'RESPUESTA_POR_CORREO') ORDER BY t.fechaModificacion DESC")
